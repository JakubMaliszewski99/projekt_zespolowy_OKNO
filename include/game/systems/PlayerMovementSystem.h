    ...
    for (auto const &entity : m_entities) {
      ...
      if (state.isMovingForward) {
        if (InputManager::getInstance()->getMouseDelta().y < 0) {
          acceleration.x += cosAngle * PLAYER_ACCELERATION / 500 *
                            -InputManager::getInstance()->getMouseDelta().y;
          acceleration.y += sinAngle * PLAYER_ACCELERATION / 500 *
                            -InputManager::getInstance()->getMouseDelta().y;
        } else {
      ...
      if (state.isMovingRight) {
        if (InputManager::getInstance()->getMouseDelta().x > 0) {
          acceleration.x -= cosAngle90 * PLAYER_ACCELERATION / 500 *
                            InputManager::getInstance()->getMouseDelta().x;
          acceleration.y -= sinAngle90 * PLAYER_ACCELERATION / 500 *
                            InputManager::getInstance()->getMouseDelta().x;
        } else {
      ...
      if (!state.isMovingForward && !state.isMovingBackwards &&
        ...
        float dampingFactor =
            std::pow(DAMPING_FACTOR, dt * DAMPING_TIME_COEFFICIENT);
      ...
      if (std::sqrt(acceleration.x * acceleration.x +
                    acceleration.y * acceleration.y) > 0.0f) {
        sf::Vector2f targetVelocity =
            normalize(acceleration) *
            std::sqrt(transform.velocity.x * transform.velocity.x +
                      transform.velocity.y * transform.velocity.y);
        transform.velocity =
            lerp(transform.velocity, targetVelocity, INTERPOLATION_FACTOR * dt);
      }

      // Clamp velocity to max speed
      if (std::sqrt(transform.velocity.x * transform.velocity.x +
                    transform.velocity.y * transform.velocity.y) >
          PLAYER_MAX_SPEED) {
        transform.velocity = normalize(transform.velocity) * PLAYER_MAX_SPEED;
      }

      // Save the acceleration value to transform component - Is it necessary
      // tho?
      ...
      if (state.isRotatingRight) {
        if (InputManager::getInstance()->getMouseDelta().x > 0) {
          transform.angle -= DEFAULT_ROTATION_SPEED / 1000 *
                             InputManager::getInstance()->getMouseDelta().x;
        } else {
      ...
      //   for (int i = subsector.firstSectorNumber; i <
      //   subsector.firstSectorNumber + subsector.segCount; i++) {
      ...
      //            25.0f, hitPoint) && (backSectorId == -1 || heightDifference
      //            >= 24)) {
      ...
      //      sf::Vector2f wallNormal = normalize({wallVector.y,
      //      -wallVector.x});

      //      // Project velocity onto the wall to prevent moving into it
      //      transform.velocity -= dot(transform.velocity, wallNormal) *
      //      wallNormal;

      //      // Slightly adjust position to avoid overlap with the wall
      //      sf::Vector2f correctionStep = wallNormal * 0.1f; // Small step
      //      away from the wall transform.positionX += correctionStep.x;
